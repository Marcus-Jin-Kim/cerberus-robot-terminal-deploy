...
def launch_setup(context, *args, **kwargs):
    ...
    nav2_bringup_cartographer_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(os.path.join(get_package_share_directory('ugv_nav'), 'launch/nav_bringup', 'CB_bringup_launch_cartographer.launch.py')),
         launch_arguments={
            'params_file': os.path.join(get_package_share_directory('ugv_nav'), 'param', 'emcl_dwa.yaml'),
            # 'configuration_basename': 'CB_localization_2d.lua', # CERBERUS EDIT: added lua configuration basename -- NOT USED ANYMORE. pure_localization not working
        }.items(),
...
def generate_launch_description():
    ...
    return LaunchDescription([
        ...
        DeclareLaunchArgument('use_localization', default_value='cartographer', description='Choose which use_localization to use: amcl,cartographer'),
